...
export function VendorModal({ isOpen, onClose, vendor, onSuccess }: VendorModalProps) {
  ...
  const loadExistingIds = async () => {
    try {
      ...
      setExistingIds(data?.map(v => v.id) || []);
    } catch (error) {
  ...
  const validateForm = () => {
    ...
    if (formData.id && existingIds.includes(formData.id) && (!vendor || vendor.id !== formData.id)) {
      setError('This ID is already assigned to another vendor');
      return false;
    }
    return true;
  };
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                {vendor ? 'Update vendor information' : 'Create a new vendor profile'}
              </p>
            </div>
          </div>
          <button
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="bg-gray-50 p-3 rounded-lg">
              ...
              {!vendor && (
                <button
                  ...
                  className="mt-1 text-sm text-green-600 hover:text-green-800 font-medium transition-colors"
                >
                  Use next available ID: {getNextAvailableId()}
                </button>
              )}
              <p className="text-xs text-gray-500 mt-1">
                Each ID can only be assigned to one vendor
              </p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all bg-gray-50"
                placeholder="0.00"
                readOnly
              />
              <p className="text-xs text-gray-500 mt-1">
        ...
        <div className="border-t border-gray-200 p-6">
          <div className="flex space-x-4">
            ...
            <button
              ...
              className="flex-1 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors disabled:opacity-50 font-medium flex items-center justify-center"
            >
              {loading ? (
                <>
