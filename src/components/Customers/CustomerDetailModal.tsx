...
export function CustomerDetailModal({ isOpen, onClose, customer, onSuccess }: CustomerDetailModalProps) {
  ...
  useEffect(() => {
    if (isOpen && customer) {
      loadTransactions();
      // Set default date range to empty (show all transactions)
  ...
  const calculateBalanceBreakdown = () => {
    ...
    // Customer formula: debit - credit = balance
  ...
  const confirmDeleteTransaction = async () => {
    ...
    try {
      ...
      onSuccess(); // Refresh customer list to update balance
  ...
  const exportToCSV = () => {
    ...
    const htmlContent = `
      ...
      <body>
        <h2>Transaction Report - ${customer.name}</h2>
        ...
        <div style="margin-top: 20px; padding: 10px; background-color: #f9fafb; border-radius: 5px;">
          ...
          <p><strong>Total Debits:</strong> <span class="debit">+Rs. ${balanceBreakdown.totalDebits.toLocaleString()}</span></p>
          <p><strong>Total Credits:</strong> <span class="credit">-Rs. ${balanceBreakdown.totalCredits.toLocaleString()}</span></p>
          <p><strong>Final Balance:</strong> <span class="${balanceBreakdown.calculatedBalance > 0 ? 'balance-positive' : balanceBreakdown.calculatedBalance < 0 ? 'balance-negative' : 'balance-zero'}">Rs. ${balanceBreakdown.calculatedBalance.toLocaleString()}</span></p>
        </div>
      </body>
      </html>
    `;
    ...
    csvLink.setAttribute('download', `customer_${customer.id}_transactions_${dateRange.startDate || 'all'}_to_${dateRange.endDate || 'all'}.csv`);
    ...
    htmlLink.setAttribute('download', `customer_${customer.id}_transactions_${dateRange.startDate || 'all'}_to_${dateRange.endDate || 'all'}.html`);
        ...
        <div className="bg-white rounded-lg p-6 w-full max-w-6xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="bg-gray-50 p-4 rounded-lg mb-6">
            <div className="flex justify-between items-center">
              <div>
                <h3 className="text-lg font-semibold text-gray-900">#{customer.id} - {customer.name}</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    >
                      ...
                      <option value="credit">Credit (Customer pays money)</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">
      ...
      {showPrintModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[70]">
          <div className="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-semibold">Customer Balance Sheet - {customer.name}</h3>
              <div className="flex space-x-2">
                <button
            ...
            <div className="print-content">
              ...
              <div className="mb-8">
                <div className="flex justify-between items-center">
                  <div>
                    <h2 className="text-lg font-semibold text-gray-900">Customer ID: #{customer.id}</h2>
